...
export default function NavBar() {
  ...
  return (
    <>
      <nav className="fixed top-0 z-50 w-full flex justify-end items-center p-4 bg-background text-foreground">
        <div className="flex items-center">
          <ModeToggle />
          <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="p-2">
            <HamburgerMenuIcon className="h-6 w-6 text-primary dark:text-primary-foreground" />
          </button>
        </div>
      </nav>
      {isMenuOpen && (
        <div className="fixed inset-0 z-40 bg-background bg-opacity-95 dark:bg-dark-background dark:bg-opacity-95 p-5 pt-20 animate-slide-in">
          <div className="flex flex-col items-end space-y-6">
            <button onClick={() => setIsMenuOpen(false)} className="absolute top-5 left-5 p-2">
              <Cross1Icon className="h-8 w-4 text-secondary dark:text-secondary-foreground" />
            </button>
